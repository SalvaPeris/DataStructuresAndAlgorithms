...
internal class Program
{
    ...
    {
        Console.WriteLine("This is a Shortest Path demo with Game Map using Dijkstra: ");
        ...
        //graph.Nodes.ForEach(n => Console.WriteLine(n));

        Node<string> source = graph.Nodes.Find(n => n.Data == "[0][0]");
        Node<string> target = graph.Nodes.Find(n => n.Data == "[16][24]");
        List<Edge<string>> path = graph.DijkstraSP(source, target);

        path.ForEach(n => Console.WriteLine(n));
